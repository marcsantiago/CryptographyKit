...
def _key_gen(plain_text, key):
	...
	key_mapper =  dict(zip(range(1,27), zip(ascii_lowercase, ascii_uppercase))) 
...
def v_encipher(plain_text, key, file_mode=False):
	...
	for index in range(len(plain)):
		...
		if plain[index].isalpha():
			...
			enciphered_stirng.append(chr(num))
		else:
			enciphered_stirng.append(plain[index])

	answer = input("Write data to file? [y], [n]\n")
	if answer.lower() in ["yes", "y"]:
		with open("enciphered_vigenere.txt", 'w') as data:
			data.write("".join(enciphered_stirng))
			return "".join(enciphered_stirng)
	else:
